<template>
  <div
    class="bg-white rounded-2xl shadow-md hover:shadow-sm transition-shadow p-4 md:p-5 border border-gray-100"
  >
    <div class="text-xs lg:text-xs text-right text-gray-500 mb-4 lg:mb-2">
      Обновлено: {{ formatDate(current.last_updated) }}
    </div>

    <div class="flex items-center flex-col sm:flex-row justify-between mb-4 sm:mb-0">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
      <div class="text-center">
        ...
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ current.humidity }}%</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Осадки</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ current.precip_mm }} мм</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Точка росы</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ current.dewpoint_c }}°</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Ветер</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ formatSpeed(current.wind_kph) }} м/с</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Порывы ветра</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ formatSpeed(current.gust_kph) }} м/с</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Давление</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ pressure }} мм рт.ст.</p>
      </div>

      <div class="text-center">
        <p class="text-xs text-gray-500 mb-1">Видимость</p>
        <p class="text-md md:text-lg font-semibold text-gray-800">{{ current.vis_km }} км</p>
      </div>
    </div>
  </div>
</template>
